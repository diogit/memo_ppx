...
let fun_memo = Str.value Nonrecursive [Vb.mk (Pat.var {txt = "fib_memo"; loc=(!default_loc)})
(Exp.apply (Exp.ident {txt = Lident "fix_memo"; loc=(!default_loc)})
[(Nolabel, Exp.ident {txt = Lident "t_fib"; loc=(!default_loc)})])]

let x functionName expression = [unwind functionName expression; fix_memo; fun_memo]
